...
class ZawgyiInputEngine : MyanmarInputEngine {
    ...
    override fun handleKeyPress(text: String, ic: InputConnection) {
        ...
        if (text == "ြ") {
            ...
            val textBefore1 = if (textBefore2.isNotEmpty()) textBefore2.last().toString() else ""

            if (textBefore2.endsWith("\u200Cေ")) {
                ic.deleteSurroundingText(2, 0)
                ic.commitText("\u200Cြေ", 1)
                return
            } else if (textBefore1 == "ေ") {
                val charBeforeE = if (textBefore2.length >= 2) textBefore2[textBefore2.length - 2].toString() else ""
                val medials = setOf("ျ", "ြ", "ွ", "ှ", "္")

                val isAttached = myanmarConsonants.contains(charBeforeE) || medials.contains(charBeforeE)
        ...
        if (myanmarConsonants.contains(text)) {
            ...
            if (textBefore2.endsWith("\u200Cြ")) {
                ic.deleteSurroundingText(2, 0)
                ic.commitText(text + "ြ", 1)
                return
            }

            if (textBefore2.endsWith("ြေ")) {
                val charBefore = if (textBefore3.length >= 3) textBefore3[textBefore3.length - 3].toString() else ""

                val isAttached = myanmarConsonants.contains(charBefore)
            ...
            if (textBefore1 == "ေ") {
                val medials = setOf("ျ", "ြ", "ွ", "ှ", "္")
                val isAttached = textBefore2.length == 2 &&
                        (myanmarConsonants.contains(textBefore2[0].toString()) ||
                                medials.contains(textBefore2[0].toString()))
            ...
            if (textBefore1 == "ြ") {
                val charBeforeYait = if (textBefore2.length >= 2) textBefore2[textBefore2.length - 2].toString() else ""
                val validYayitConsonants = setOf("က", "ခ", "ဂ", "င", "ပ", "ဖ", "ဗ", "ဘ", "မ", "သ")
                val isAttached = validYayitConsonants.contains(charBeforeYait)
